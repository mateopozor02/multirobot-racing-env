...
class CrowdSim(gym.Env):
    ...
    def generate_static_human(self):
        ...
        human.v_pref = 0
        while True:
            px = random.uniform(-self.circle_radius * 1.5 / 2.0, self.circle_radius * 1.5 / 2.0)
        ...
        if self.config.get('humans', 'policy') == 'trajnet':
            ...
            if self.case_counter[phase] >= 0:
                ...
                if phase in ['train', 'val']:
                    ...
                    self.generate_random_human_position2(human_num=human_num, rule=self.train_val_sim)

                else:
                    self.generate_random_human_position2(human_num=self.human_num, rule=self.test_sim)
    ...
    def step(self, action, update=True):
        ...
        if self.global_time >= self.time_limit - 1:
            ...
            #reward = 0
            #reward = - (end_dg*end_dg*end_dg*end_dg*end_dg)/(start_dg*start_dg*start_dg*start_dg*start_dg)* self.time_step
            reward = (1 - end_dg/start_dg) * self.time_step # function 1
            # reward = 1 - 0.16*end_dg*end_dg # function 2
            # reward = 1 / (1 + np.exp(end_dg - 3)) #function 3
            # reward = 1 / (1 + np.exp(2.5*end_dg - 4)) #function 4
            # reward = 1 / (1 + np.exp(end_dg - 4)) #function 5
            #reward = 2 * (start_dg - end_dg)  * self.time_step
            done = False
            info = Nothing()
    ...
    def render(self, mode='human', output_file=None):
        ...
        if mode == 'human':
            ...
            # add time annotation
            time = plt.text(-1, 11, 'Time: {}'.format(0), fontsize=16)
            ax.add_artist(time)
