...
class RobotAction(object):
    ...
    def robot_vel_on_map_calculator(self, msg):
        ...
        #listener_v.waitForTransform('/map', '/sarl_tb3_tf/base_footprint', rospy.Time(0), rospy.Duration(10))
        #trans, rot = listener_v.lookupTransform('/map', '/sarl_tb3_tf/base_footprint', rospy.Time(0))
    ...
    def get_goal_on_map(self, msg):
        ...
        #listener_g.waitForTransform('/map', '/sarl_tb3_tf/odom', rospy.Time(0), rospy.Duration(10))
...
if __name__ == '__main__':
    ...
    # set file dirs
    pack_path = rospkg.RosPack().get_path('sarl_star_ros')
    #model_dir = pack_path + '/CrowdNav/crowd_nav/data/output_new1_0.4_f1/'
    model_dir = pack_path + '/CrowdNav/crowd_nav/data/output_alisher_ICCAS2023/'
    env_config_file = model_dir + 'env.config'
    policy_config_file = model_dir + 'policy.config'
    if os.path.exists(os.path.join(model_dir, 'resumed_rl_model.pth')):
        model_weights = os.path.join(model_dir, 'resumed_rl_model.pth')
    ...
    try:
        ...
        # Publish the reward to 'calculated_reward' topic
        pub = rospy.Publisher('calculated_reward', Float32, queue_size=1000)

        # Publish the wall min distance reward to 'wall_min_distance' topic
        pub_wall = rospy.Publisher('wall_min_distance', Float32, queue_size=1000)

        # Publish the keep moving reward to 'keep_moving' topic
        pub_keep_moving = rospy.Publisher('keep_moving', Float32, queue_size=1000)

        # Publish the r_for reward to 'r_for' topic
        pub_r_for = rospy.Publisher('r_for', Float32, queue_size=1000)

        # Publish the r_danger reward to 'r_danger' topic
        pub_r_danger = rospy.Publisher('r_danger', Float32, queue_size=1000)

        # Publish the r_forward reward to 'r_forward' topic
        pub_r_forward = rospy.Publisher('r_forward', Float32, queue_size=1000)

        # Publish the r_collision reward to 'r_collision' topic
        pub_r_collision = rospy.Publisher('r_collision', Float32, queue_size=1000)

        # Publish the r_collision_wall reward to 'r_collision_wall' topic
        pub_r_col_wall = rospy.Publisher('r_col_wall', Float32, queue_size=1000)

        # Publish the r_stop_t reward to 'r_stop_t' topic
        pub_r_stop_t = rospy.Publisher('r_stop_t', Float32, queue_size=1000)

        # Publish the accumulated reward to 'accumulated_reward' topic
        pub_accumulated_reward = rospy.Publisher('accumulated_reward', Float32, queue_size=1000)

        # Publish the r_goal reward to 'r_goal' topic
        pub_r_goal = rospy.Publisher('r_goal', Float32, queue_size=1000)
        ...
        while not rospy.is_shutdown():
            ...
            wall_min_dist_reward = policy.r_wall_min_dist
            ros_wall_min_dist_reward = Float32()
            ros_wall_min_dist_reward = wall_min_dist_reward

            keep_moving_reward = policy.r_km 
            ros_keep_moving_reward = Float32()
            ros_keep_moving_reward = keep_moving_reward

            r_for_reward = policy.r_for
            ros_r_for_reward = Float32()
            ros_r_for_reward = r_for_reward

            r_danger_reward = policy.r_danger
            ros_r_danger_reward = Float32()
            ros_r_danger_reward = r_danger_reward

            r_forward_reward = policy.r_forward
            ros_r_forward_reward = Float32()
            ros_r_forward_reward = r_forward_reward

            r_collision_reward = policy.r_collision
            ros_r_collision_reward = Float32()
            ros_r_collision_reward = r_collision_reward

            r_col_wall_reward = policy.r_col_wall
            ros_r_col_wall_reward = Float32()
            ros_r_col_wall_reward = r_col_wall_reward

            r_stop_t_reward = policy.r_stop_t
            ros_r_stop_t_reward = Float32()
            ros_r_stop_t_reward = r_stop_t_reward

            accumulated_reward = policy.accumulated_reward
            ros_accumulated_reward = Float32()
            ros_accumulated_reward = accumulated_reward

            r_goal_reward = policy.r_goal
            ros_r_goal_reward = Float32()
            ros_r_goal_reward = r_goal_reward
            ...
            #Publish the reward to 'calculated_reward' topic
            pub.publish(ros_reward)
            pub_wall.publish(ros_wall_min_dist_reward)
            pub_keep_moving.publish(ros_keep_moving_reward)
            pub_r_for.publish(ros_r_for_reward)
            pub_r_danger.publish(ros_r_danger_reward)
            pub_r_forward.publish(ros_r_forward_reward)
            pub_r_collision.publish(ros_r_collision_reward)
            pub_r_col_wall.publish(ros_r_col_wall_reward)
            pub_r_stop_t.publish(ros_r_stop_t_reward)
            pub_accumulated_reward.publish(ros_accumulated_reward)
            #print("Accumulated reward is: ", ros_accumulated_reward)
            pub_r_goal.publish(ros_r_goal_reward)
            rate.sleep()
