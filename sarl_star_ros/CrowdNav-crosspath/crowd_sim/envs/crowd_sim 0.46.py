...
class CrowdSim(gym.Env):
    ...
    def __init__(self):
        ...
        """
	self.min_dist_coordinates = []
	self.max_dist_coordinates = []
	...
	self.crossed_first = False
	self.crossed_second = False
	self.m1 = None
	self.b1 = None
	self.m2 = None
	self.b2 = None
    ...
    def configure(self, config):
        ...
        self.block_area1 = [[2,9],[-2,9],[-2,5],[2,5]]
        self.block_area2 = [[2.5,2.5],[-2.5,2.5],[-2.5,-2.5],[2.5,-2.5]]
        self.block_area3 = [[2,-5],[-2,-5],[-2,-9],[2,-9]]
        self.block_area4 = [[-5,2],[-9,2],[-9,-2],[-5,-2]]
        self.block_area5 = [[9,2],[5,2],[5,-2],[9,-2]]
	self.blocks = [self.block_area1, self.block_area2, self.block_area3, self.block_area4, self.block_area5]
    ...
    def generate_fake_human(self):
        ...
        while True:
            ...
            if (px<self.block_area4[0][0]+human.radius and px>self.block_area4[2][0]-human.radius) and (py<self.block_area4[0][1]+human.radius and py>self.block_area4[2][1]-human.radius):
                collide = True
            if (px<self.block_area5[0][0]+human.radius and px>self.block_area5[2][0]-human.radius) and (py<self.block_area5[0][1]+human.radius and py>self.block_area5[2][1]-human.radius):
                collide = True
            if not collide:
    ...
    def generate_circle_crossing_human(self):
        ...
        while True:
            ...
            px = (self.circle_radius-1) * np.cos(angle) + px_noise
            py = (self.circle_radius-1) * np.sin(angle) + py_noise
    ...
    def generate_square_crossing_human(self):
        ...
        while True:
            ...
            if py<=self.block_area1[0][1]+human.radius and py>=self.block_area1[2][1]-human.radius:
                collide = True
            elif py<=self.block_area2[0][1]+human.radius and py>=self.block_area2[2][1]-human.radius:
                collide = True
    ...
    def reset(self, number, phase='test', test_case=None):
        ...
        if self.config.get('humans', 'policy') == 'trajnet':
            ...
            self.robot.set(4, -4, -4, 4, 0, 0, np.pi / 4 * 3)
    ...
    def step(self, action, update=True):
        ...
        if (next_px<self.block_area4[0][0]+self.robot.radius and next_px>self.block_area4[2][0]-self.robot.radius) and (next_py<self.block_area4[0][1]+self.robot.radius and next_py>self.block_area4[2][1]-self.robot.radius):
            collision_wall = True
        if (next_px<self.block_area5[0][0]+self.robot.radius and next_px>self.block_area5[2][0]-self.robot.radius) and (next_py<self.block_area5[0][1]+self.robot.radius and next_py>self.block_area5[2][1]-self.robot.radius):
            collision_wall = True
	...
	reward = R_danger + 2*R_stop +R_forward+ R_goal + R_collision + R_km + R_col_wall + timeout + R_wall_min_dist + R_wall_max_dist + R_for + R_stop_t
	reward_values = {"Total Reward": reward,"R_dan": R_danger,"2*R_stop": 2*R_stop, "R_for": R_forward, "R_goal": R_goal,"R_col": R_collision,"R_km": R_km, "R_col_wall": R_col_wall, "timeout": timeout, "R_wall_min_dist": R_wall_min_dist, "R_wall_max_dist": R_wall_max_dist, "R_forward": R_for, "R_stop_t": R_stop_t}      
    ...
    def render(self, mode='human', output_file=None):
        ...
        if mode == 'human':
            ...
            for i in range(len(self.block_area1)):
                ...
                plt.plot([self.block_area5[i][0],self.block_area5[(i+1)%len(self.block_area5)][0]],[self.block_area5[i][1],self.block_area5[(i+1)%len(self.block_area5)][1]], color = 'red')
