...
class CrowdSim(gym.Env):
    ...
    def configure(self, config):
        ...
        self.block_area2 = [[2,2],[-2,2],[-2,-2],[2,-2]]
    ...
    def step(self, action, update=True):
        ...
        reward = 0
        if (start_dg - end_dg) > 0:
            reward += self.heading_reward
        else:
            reward += -self.heading_reward
        if position_variation > self.standard_variation:
            reward += self.maintaining_reward
        else:   
            reward += -self.maintaining_reward
        if self.global_time >= self.time_limit - 1:
            ...
            reward += self.collision_wall_penalty
            done = True
            info = CollisionWall()
        elif reaching_goal:
