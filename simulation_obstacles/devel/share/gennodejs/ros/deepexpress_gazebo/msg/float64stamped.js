...
// (in-package deepexpress_gazebo.msg)
...
class float64stamped {
  ...
  static deserialize(buffer, bufferOffset=[0]) {
    ...
    for (let i = 0; i < len; ++i) {
      data.data[i] = std_msgs.msg.Float64.deserialize(buffer, bufferOffset)
    }
    return data;
  }
  ...
  static Resolve(msg) {
    ...
    if (msg.data !== undefined) {
      ...
      for (let i = 0; i < resolved.data.length; ++i) {
        resolved.data[i] = std_msgs.msg.Float64.Resolve(msg.data[i]);
      }
    }
    else {
      resolved.data = []
